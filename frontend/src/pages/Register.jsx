...
export function Register() {
  ...
  async function submit(e) {
    ...
    try {
      ...
      if (role === 'vendor') payload.storeName = storeName
  ...
  return (
    <div className="mx-auto max-w-md rounded-3xl border border-[var(--color-border)] bg-white/5 p-6">
      <div className="text-xl font-black">Create account</div>
      <div className="mt-1 text-sm text-white/60">Register as a buyer or vendor.</div>
      ...
      <form onSubmit={submit} className="mt-5 space-y-3">
        <label className="space-y-1 block">
          <div className="text-xs font-bold text-white/70">Role</div>
          <select
            value={role}
        ...
        <label className="space-y-1 block">
          <div className="text-xs font-bold text-white/70">Name</div>
          <input
            value={name}
        ...
        {role === 'vendor' ? (
          <label className="space-y-1 block">
            <div className="text-xs font-bold text-white/70">Store name</div>
            <input
              value={storeName}
